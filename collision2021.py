                            ...
                            for j in range(Nv):                      
                                    for i in range(Nv):
                                            ...
                                            if i==Nv-1 and j!=Nv-1 and j!=0:
                                                    f_temp1[(r)*(Nv)*(Nv)+j*Nv+i]=3*f_1[(r)*(Nv)*(Nv)+(j-1)*Nv+i-1]-f_1[(r)*(Nv)*(Nv)+(j)*Nv+i-2]-f_1[(r)*(Nv)*(Nv)+(j-2)*Nv+i]#f_1[(r)*(Nv)*(Nv)+(j)*Nv+i-1]*d_pal_po[r*(Nv)+j] #np.max(f_1)*10**(2*np.log10(f_1[(r)*(Nv)*(Nv)+(j)*Nv+i-1]/np.max(f_1))-np.log10(f_1[(r)*(Nv)*(Nv)+(j)*Nv+i-2]/np.max(f_1)))                                  #((pal_v[i]-pal_v[i-2])/(pal_v[i-2]-pal_v[i-1]))*(f_1[(r)*(Nv)*(Nv)+j*Nv+i-2]-f_1[(r)*(Nv)*(Nv)+j*Nv+i-1])+f_1[(r)*(Nv)*(Nv)+j*Nv+i-2] 
                                            if j==0:
